...
export default {
  name: "dec-product-table-template",
  props: {
    title: String,
    search: String
  },
  ...
  computed: {
    ...mapGetters('decProducts', ["decProducts"]),
  },

  methods: {
    ...mapActions("decProducts", ["handleGetDecProducts", "handleDeleteDecProduct", "handleSelectedDecProduct"]),

    initialize() {
      this.handleGetDecProducts();
    },

    editItem(item) {
      this.isEdit = true;
      this.handleSelectedDecProduct(item)
      this.$emit('handleToggleDialog', true)
    },

    deleteItem(item) {
      confirm("Are you sure you want to delete this item?") &&
        this.handleDeleteDecProduct(item._id);
    },
  }
};
</script>
